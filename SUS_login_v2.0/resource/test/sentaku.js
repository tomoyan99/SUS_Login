...
const inputKey = (state) => new Promise(resolve => {
	...
	const callBack = key => {
		...
		try {
			...
			// console.log(`${startLine(state.start + state.option.length -1, 8)}${state.index}${rightClear}`);//選択している項目の表示
		} catch (e) {
...
// 標準入力の受け取り
async function choice(prompt="選択?>" ,state={option:[""],index:1,start:1}) {
	let key = "";
...
(async () => {
	console.clear();
	let state = {
		option: ["ダミー","1.sclass", "2.sola", "3.sclassとsola", "4.euc","5.aaa","6.bbb","7.ccc"],
		index: 1,//選択している行数
		start: 1 //開始行数
	};
	...
	console.log(startLine(state.start + state.option.length + 1)+"選択したのは"+state.option[state.index]+"です");
})();