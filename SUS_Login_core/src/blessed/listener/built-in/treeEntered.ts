...
export function treeEntered(self: UIManager, tree: contrib.widget.Tree) {
    ...
    if (node.name && node.event) {
        ...
        let colored_content = `>>{${self.config.colors.choice.fg}-fg}{${self.config.colors.choice.bg}-bg}${content}{/}{/}`;
        //イベントをエミット
        ...
        try {
            ...
            self.eventService.emit(node.event, self, node);
        } catch (e) {
