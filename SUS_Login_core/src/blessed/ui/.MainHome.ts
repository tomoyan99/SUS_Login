...
class Components extends Members {
     ...
     * */
    private makeNet(grid: contrib.grid): Widgets.TextElement | void {
        if (!this.position.net) return;
     ...
     * */
    private makeChoice(grid: contrib.grid): Widgets.TextElement | void {
        if (!this.position.choice) return;
        return grid.set(...this.position.choice, Blessed.text, {
                keys: false, // キー入力
                parent: grid.screen, // 必ず指定
                name: "choice",
                label: "CHOICE", // 表示する名称
     ...
     * */
    private makeInfo(grid: contrib.grid):Widgets.BoxElement|void {
        if (!this.position.info)return;
     ...
     * */
    private makeForm(grid: contrib.grid): Widgets.TextboxElement|void {
        if (!this.position.form)return;
     ...
     * */
    private makeMainTree(screen: Widgets.Screen, grid: contrib.grid):contrib.widget.Tree|void {
        if (!this.position.mainTree) return;
     ...
     * */
    private makeSubTree(grid: contrib.grid): contrib.widget.Tree|void {
        if (!this.position.subTree)return;
