...
export async function makeSchedule(data) {
	...
	const browser = await launch({
		...
		headless: true, //ヘッドレス(ブラウザの表示・非表示)の設定。falseなら表示
...
async function searchSclass(browser, data) {
	...
	try {
		...
		async function evalSUS(classname,content,page) {
			const path_bf = `table#form1\\3A standardJugyoTimeSchedule00List td.${classname}`;
			const path_af = `table#form1\\3A standardJugyoTimeSchedule01List td.${classname}`;
			const array = {
				"bf": Array.from(new Set(
					await page.$$eval(path_bf, (tds) => { return tds.map(data => data[content]) })
				)),
				"af": Array.from(new Set(
					await page.$$eval(path_af, (tds) => { return tds.map(data => data[content]) })
				))
			};
			return array;
		}
		...
		//授業コードを取得
		let class_code = await evalSUS("jugyoCd span a","textContent",page);

		//授業名を取得
		let class_name = await evalSUS("jugyoMei span a","textContent", page);
