...
import {appendFileSync, existsSync, readFileSync, writeFileSync} from 'fs'; //fsのインポート
...
import {makeSchedule} from './util/makeSchedule.js';//sola_link.jsonの作成
...
export async function login(version = "") {
    ...
    */
    ...
    data.internet.onLine = await isNetConnected();
    data.internet.status = (await isNetConnected())?`${cl.bg_green}良好${cl.bg_reset}`:`${cl.bg_red}不良${cl.bg_reset}`
    ...
    let whichOpen;
    //一定時間操作がなければプロセスを終了
    const timeout = {now:new Date().valueOf()};//基準時間
    const limit_sec = 24 * 60 * 60 * 1000;//制限時間 時 分 秒 ミリ秒(デフォルトで40分)
    setInterval(()=>{
        // console.log((new Date().valueOf() - timeout.now) / 1000);
        if (new Date().valueOf() - timeout.now > limit_sec ){
    ...
    selectcommand:
        do {
            ...
            let main_option = {
                home:{
                    head: [
                        ...
                        `\tインターネット接続状況：${data.internet.status}\n`+
                        `${(data.internet.onLine)?``:`${cl.fg_yellow}※ ネットワークが繋がっていないため${cl.fg_red}QUIT${cl.fg_reset}以外の操作を拒絶中です！${cl.fg_reset}\n`}`+
                        `\n`],
                    main: [["1.euc", ">>logs", ">>images"], ["2.sola", ">>List"], ["3.sclass"], ["4.sclassとsola"], [`${cl.fg_red}5.QUIT${cl.fg_reset}`]],
                    bottom: ["選択?>"],
                    networkConnected:async ()=>await isNetConnected()
                },
                bf:{},
                af:{}
            }
            ...
            network_ID = setInterval(async()=>{
                const nowNetwork = await isNetConnected()
                //0.1秒に一回接続状況に変化があるか確認。なければ何もしない
                if (data.internet.onLine !== nowNetwork){
                    data.internet.onLine = nowNetwork;
                    data.internet.status = (nowNetwork)?`${cl.bg_green}良好${cl.bg_reset}`:`${cl.bg_red}不良${cl.bg_reset}`;
                    main_option.home.head= [
                            `SUS_login_v${version}\n`+
                            `${cl.fg_green}十字キーで開きたい項目を選択してね${cl.fg_reset}`+
                            `\tインターネット接続状況：${data.internet.status}\n`+
                            `${(data.internet.onLine)?``:`${cl.fg_yellow}※ ネットワークが繋がっていないため${cl.fg_red}QUIT${cl.fg_reset}以外の操作を拒絶中です！${cl.fg_reset}\n`}`+
                    ...
                    if (main_option.bf !== {} || main_option.af !== {}){
                        for (const key in main_option.bf) {
                            main_option.bf[key]=main_option.home[key]
                            main_option.af[key]=main_option.home[key]
                        }
                    }
                }
            },1);
            ...
            //コマンドリスト
            let comlist = [[["euc"], ["logs"], ["images"]], [["sola"], ["solalist"]], [["sclass"]], [["sclass", "sola"]], [["quit"]]];
            //コマンドリストの中からどれを選んだか
            whichOpen = comlist[state.chosing().index[0]][state.chosing().index[1]];
            //QUITを選択した場合は終了
            if (whichOpen[0] === "quit") {
                break;
            }
            //ネットワークが繋がっていない場合はQUIT以外何もできないようにする
            if (data.internet.onLine === true){
                whichOpen = comlist[state.chosing().index[0]][state.chosing().index[1]];
            }else{
                whichOpen = ["networkerr"];
            }
            if (whichOpen[0] === "networkerr"){
                continue;
            }
            ...
            if (whichOpen[0] === "logs") {
                ...
                await pause("[エンターを押して選択に戻る]");
                continue;
            }
            if (whichOpen[0] === "images") {
            ...
            if (whichOpen[0] === "solalist") {
                ...
                main_option.bf.main.push([`${cl.fg_yellow}<<戻る${cl.fg_reset}`, `${cl.fg_green}>>後期ページへ${cl.fg_reset}`]);
                main_option.af.main.push([`${cl.fg_yellow}<<戻る${cl.fg_reset}`, `${cl.fg_green}>>前期ページへ${cl.fg_reset}`]);
                //前期を先に開く
                state = await choice(main_option.bf);
                selectpage:
                while (!state.networkChenged) {
                    //ページの選択をしたときはループ
                    switch (state.chosing().str) {
                        case ">>後期ページへ":
...
async function openSclass(browser, data) {
    ...
    const waitAccess = setInterval(()=>{
        ...
        if (dot_count === data.access_dot.max_quant){
            process.stdout.write(`\x1b[${dot_count}D${cl.rightClear}`);
            dot_count=0;
        }
    },data.access_dot.waitmsec);

    await page.goto(url, {waitUntil: 'networkidle2', timeout: 0}); //ページ遷移

    //アクセスが完了したらドットを打つのをやめてアクセス完了の文字を出力
    process.stdout.write(`${cl.lineClear}${cl.goLineHead}${cl.fg_green}アクセス完了${cl.fg_reset}\n`);
    ...
    try {
        await page.waitForSelector("div#htmlErrorMessage", {visible: true, timeout: 2000});
...
async function openEuc(browser, data, EUC) {
    ...
    await page.waitForSelector(target_euc1_ID, {visible: true, timeout: 30000});
    await page.click(target_euc1_ID);//EUCをクリック
