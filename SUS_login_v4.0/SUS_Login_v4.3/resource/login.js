...
export async function login(version="") {
	...
	*/
	...
	do {
		//選択用のargs
		let main_option = {
			head: [`SUS_login_v${version}\n${cl.fg_green}十字キーで開きたい項目を選択してね${cl.fg_reset}\n\n`],
			main: [["1.euc", ">>logs", ">>images"], ["2.sola", ">>List"], ["3.sclass"], ["4.sclassとsola"],["5.QUIT"]],
			bottom: ["選択?>"]
		}
		//選択
		...
		if (whichOpen[0] == "solalist") {
			...
			while (true) {
				...
				} else if (state.chosing().str === `${cl.fg_green}>>前期ページへ${cl.fg_reset}`) {
					state = await choice(main_option.bf);
				} else {
...
async function openSclass(browser,data) {
	...
	await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 0 }).catch(async () => {
		console.log(`${cl.bg_red}接続エラー：接続に失敗しました。${cl.bg_reset}`);
		await browser.close();
		return;
	}); //ページ遷移
...
async function openEuc(browser,data,EUC) {
	...
	try {
		...
		console.log(`${cl.bg_red}接続エラー(${data.miss_count+1})：接続に失敗しました再試行します。${cl.bg_reset}`);
