...
export function existChromePath(): boolean {
  ...
  const prefixes = <string[] | []>(
    [process.env.LOCALAPPDATA, process.env.PROGRAMFILES, process.env["PROGRAMFILES(X86)"]].filter(
      Boolean,
    )
  ); //空の値を削除
  //ファイルが存在するかを各パスに確認
  if (prefixes && prefixes.length > 0) {
    const existList = prefixes.map((prefix) => existsSync(join(prefix, suffix))).filter(Boolean);
